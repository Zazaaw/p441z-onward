...
const url = process.env.AGHRIS_SUPABASE_URL;
const serviceKey = process.env.AGHRIS_SUPABASE_SERVICE_ROLE_KEY;

if (!url || !serviceKey) {
  throw new Error(
    "Env AGHRIS belum lengkap. Butuh AGHRIS_SUPABASE_URL dan AGHRIS_SUPABASE_SERVICE_ROLE_KEY di .env.local"
  );
}
...
export const MY_NIK = process.env.AGHRIS_NIK ?? "3028226";
